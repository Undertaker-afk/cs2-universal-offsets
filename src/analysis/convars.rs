...
pub fn convars<P: Process + MemoryView>(
    ...
    let cvar_ptr_addr = sig_hits
        .get("CVar_ptr")
        .ok_or_else(|| anyhow!("CVar_ptr signature not found"))?;

    let cvar_inst = process
        .read_ptr(Pointer64::<u64>::from(*cvar_ptr_addr))
        .data_part()?;
    if cvar_inst.is_null() {
    ...
    while !node_ptr.is_null() && count < 20000 {
        ...
        let name = process
            .read_utf8_lossy(node.name.address(), 128)
            .data_part()?;
        if name.is_empty() {
            break;
        }
        ...
        let description = process
            .read_utf8_lossy(node.description.address(), 256)
            .data_part()?;
        ...
        if show_values {
            ...
            if let Ok(var_inst) = process.read_ptr(var_ptr).data_part() {
                if !var_inst.is_null() {
                    if let Ok(s) = process
                        .read_utf8_lossy(Address::from(var_inst), 64)
                        .data_part()
                    {
                        val_str = s;
                    }
